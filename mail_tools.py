...
from main_startup import Tezer
...
**Body :** {body}"""
    if not is_file:
        if len(last) > 1024:
            file_names = "email.text"
            open(file_names, "w").write(last)
            await Tezer.send_document(Config.LOG_GRP, file_names, caption = "Your Mail")
            os.remove(file_names)
        else:
        ...
        if len(last) > 1024:
            await Tezer.send_document(Config.LOG_GRP, fl_name, caption = "Your Mail Attachment")
            await Tezer.send_document(Config.LOG_GRP, file_names, caption = "Your Mail")
            os.remove(fl_name)
            os.remove(file_names)
        else:
            await Tezer.send_document(Config.LOG_GRP, fl_name, caption = last)
            os.remove(fl_name)
