...
from main_startup import bot, Tezer, Config
...
async def job_close():
    ...
    for warner in lol:
        try:
            await Tezer.send_message(
              int(warner.get("chat_id")), "`12:00 Am, Group Is Closing Till 6 Am. Night Mode Started !` \n**Powered By @TezerOT**"
            )
            await Tezer.set_chat_permissions(warner.get("chat_id"), ChatPermissions())
            async for member in Tezer.iter_chat_members(warner.get("chat_id")):
             if member.user.is_deleted:
                try:
                    await Tezer.kick_chat_member(warner.get("chat_id"), member.user.id)
                except:
                    pass
            ...
            try:
                await Tezer.send_message(Config.LOG_GRP, f"[NIGHT MODE]\n\nFailed To Close The Group {ido}.\nError : {e}")
            except:
                pass
...
async def job_open():
    ...
    for warner in lol:
        try:
            ...
            await Tezer.set_chat_permissions(
                        warner.get("chat_id"),
                        ChatPermissions(
